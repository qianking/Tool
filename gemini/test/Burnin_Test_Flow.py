...
class Terminal_Server(metaclass = mChild):
    ...
    def __init__(self, terminal_comport):
        ...
        self.Variable.logger = file_util.create_logger(logger_path, 'main_log')
        self.test = Test_Item.Terminal_Server_Test_Item(port = self.terminal_comport, baud = self.baud, Variable = self.Variable)
        #super().__init__(port = self.terminal_comport, baud = self.baud)  

    def Check_All_Comport(self):
        """
        確認terminal server和pg的port口是否正常
        """
        self.test.Check_Comport()
    
    def Terminal_Server_Flow(self):
        """
        terminal server清線
        """
        self.test.Enter_en_Mode()
        self.test.Clear_Port_on_Terminal(2, 10)
