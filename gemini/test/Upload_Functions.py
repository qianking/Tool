...
from exceptions import Upload_Fail

logger_path = r'.\debug'

class Upload_FTP():

    FTP_logger = create_log.create_logger(logger_path, 'FTP_log')
    upload_flag = True
    
    def __init__(self, ftp_upload_path, Variable):
        self.Variable = Variable
        self.ftp = FTP_UP_Down(self.FTP_logger)
...
class SFIS_Function():
    ...
    SFIS_logger = create_log.create_logger(logger_path, 'SFIS_log')

    def __init__(self, sfis_op, sfis_deviceID, Variable):
        self.Variable = Variable
        self.sfis=SFIS() 
        self.sfis_deviceID = sfis_deviceID
        self.sfis_op = sfis_op
        self.TSP = 'Burnin'
    ...
    def SFIS_Checkroute_Thread(self):
        ...
        with ThreadPoolExecutor(max_workers=len(dut_SSN_dic)) as executor:
            for port, dut_ssn in dut_SSN_dic.items():
                futures = executor.submit(self.sfis_checkroute, dut_ssn, self.sfis_deviceID[port])
    ...
    def sfis_checkroute(self, SSN, deviceID):
        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] get SSN: {SSN} ,deviceID: {deviceID}')
        
        retry_times = 0
        
        sfis_data = self.sfis.Logout(self.sfis_op, deviceID, self.TSP)
        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] logout: {sfis_data}')
        sfis_data = self.sfis.Login(self.sfis_op, deviceID, self.TSP)
        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] login: {sfis_data}')
        while True:
            sfis_data = self.sfis.CheckRoute(SSN, deviceID)
            self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute: {sfis_data}')
            if sfis_data:
                if int(sfis_data[0]) == 1:
                    self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute >>PASS<<')
                    return 0
                else:
                    if "WRONG STEP" in sfis_data[1]:
                        sfis_error_msg = f'進錯站: {sfis_data[1]}'
                        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute >>WRONG STEP<< : {sfis_data[1]}')
                        if retry_times < 1:
                            retry_times += 1
                            self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute retry: {retry_times}')
                            continue
                        else:
                        ...
                        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute >>ISN NOT INPUT<< : {sfis_data[1]}')
                        if retry_times < 1:
                            retry_times += 1
                            self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute retry: {retry_times}')
                            continue
                        ...
                        self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute >>FAIL<< : {sfis_data[1]}')
                        if retry_times < 1:
                            retry_times += 1
                            self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute retry: {retry_times}')
                            continue
                ...
                self.SFIS_logger.debug(f'[SFIS check route] in [{deviceID}] chekroute >>FAIL DATA ERROR<< :{sfis_data[1]}')
    ...
    def sfis_upload(self, SSN, error, deviceID, sfis_data, status = 1):
        self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] get SSN: {SSN} , error: {error}, deviceID: {deviceID}, sfis_data: {sfis_data}')
        retry_times = 0
        data = str()
        data2 = str()
        data3 = str()
        ...
        self.sfis.Logout(self.sfis_op, deviceID, self.TSP)
        self.sfis.Login(self.sfis_op, deviceID, self.TSP)
        ...
        while True:
            sfis_data = self.sfis.UploadRawData(SSN, error, deviceID, self.TSP, status, data_list)
            if sfis_data:
                if int(sfis_data[0]) == 1:
                    self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] upload >>PASS<<')
                    print('SFIS upload success')
                    return 0
                else:
                    if "WRONG STEP" in sfis_data[1]:
                        sfis_error_msg = f'進錯站: {sfis_data[1]}'
                        self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] upload >>WRONG STEP<< :{sfis_data[1]}')
                        print('進錯站')
                        if retry_times < 1:
                            retry_times += 1
                            self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] upload retry: {retry_times}')
                            continue
                        else:
                            self.Variable.sfis_error_msg = sfis_error_msg
                            raise Upload_Fail
                        ...
                        self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] upload >>FAIL<< :{sfis_data[1]}')
                ...
                self.SFIS_logger.debug(f'[SFIS upload] in [{deviceID}] upload >>FAIL SFIS DATA ERROR<< :{sfis_data[1]}')
