    ...
    def __new__(cls, name, bases, local):
        for attr in local:
            ...
            if not len(bases) and callable(value) and attr != '__init__':
                local[attr] = log_deco(local['Variable'])(value)
...
class log_deco():
    ...
    def __call__(self, func):
        ...
        def decorated(*args, **kwargs):
            ...
            try:
                ...
                self.Variable.logger.debug(f'>>>>> In [{test_item}] <<<<<')
    ...
    def error_function(self, test_item, test_item_data = None):
        ...
        self.Variable.logger.debug(f'>>>>> Failed In [{test_item}] <<<<<')
        self.Variable.raw_log = {'end_time': time.time()}
    ...
    def sys_exception(self, ex):
        ...
        if 'FileNotFoundError' in str(ex):           #console連接錯誤 電腦找不到這個port口
            ...
            self.Variable.sys_error_msg = str(ex)

          
class Terminal_Server_Test_Item(metaclass = myMetaClass):

    Variable = Global_Variable.All_Variable(Log_Model.no_name_time)
