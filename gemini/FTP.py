...
logger_path = r'.\debug'
ftb_logger = file_util.create_logger(logger_path, 'ftb_log')
...
class FTP_UP_Down():
    ...
    def connect_ftp(self, ip, port, user, password):
        ...
        try:
            ...
            ftb_logger.exception(f"cannot reach {ip}")
            raise Exception(f"ERROR: cannot reach {ip}")
        except error_perm:
            ftb_logger.exception("cannot login")
            raise Exception("ERROR: cannot login")

    @staticmethod
    def check_localfile_exists(localpath):
        if not os.path.isfile(localpath):
            ftb_logger.exception(f"cannot find local path: {localpath}")
            raise Exception("本地端找不到文件")

    @staticmethod
    def check_localdir_exists(localdir):
        if not os.path.isdir(localdir):
            ftb_logger.exception(f"cannot find local folder: {localdir}")
            raise Exception("本地端找不到資料夾")

    @staticmethod
    def create_localdir(localdir):
        if not os.path.isdir(localdir):
            os.makedirs(localdir)
    
    def check_open_remotedir(self, remotedir):
        try:
            ftb_logger.info(f"open FTP folder: {remotedir}")
            self.ftp.cwd(remotedir)
        except:
            ftb_logger.exception(f"cannot find FTP folder: {remotedir}")
    ...
    def create_open_remote_file(self, remotedir):
        ...
        for file in new_remotepath:                                                            
            try:
                self.ftp.cwd(file)
                ftb_logger.info(f"open FTP folder: {file}")                             
            except Exception as ex:  
                try:
                    self.ftp.mkd(file)
                    ftb_logger.info(f"create FTP folder: {file}")   
                    self.ftp.cwd(file)
                    ftb_logger.info(f"open FTP folder: {file}")
                except Exception as ex:
                    ftb_logger.exception(f"create file exception: {ex}")
                    self.ftp.cwd(file)
                    ftb_logger.info(f"open FTP folder: {file}")
    ...
    def UploadFile(self, localfile, remotedir):
        ...
        try:
            ...
            ftb_logger.exception(f"exception:") 
            raise Exception(ex)
        else:
            if '226' in res:
                ftb_logger.info(f"{filename} upload finished")   
    ...
    def DownloadFile(self, remotefile, localdir):
        ...
        try:
            ...
            if '226' in res:
                ftb_logger.info(f"{local_filename} download finished")  
